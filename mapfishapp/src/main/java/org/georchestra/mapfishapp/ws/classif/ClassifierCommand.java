...
public class ClassifierCommand {
    ...
    /**
     * All following members are designed to map names used by the client side and
     * the java code
     */
    ...
    public enum E_ClassifType {
        CHOROPLETHS, PROP_SYMBOLS, UNIQUE_VALUES
    };
        ...
        try {
            ...
            if (_classifType == E_ClassifType.CHOROPLETHS) {
                try {
                    ...
                    throw new DocServiceException("Colors should be given in hex format",
                            HttpServletResponse.SC_BAD_REQUEST);
                }
                _classCount = jsonRequest.getInt(JSON_CLASSCOUNT);
                if (_classCount <= 0) {
                    throw new DocServiceException("Number of classes cannot negative",
                            HttpServletResponse.SC_BAD_REQUEST);
                }
                ...
                if (_maxSize <= _minSize) {
                    throw new DocServiceException("Maximum size cannot be greater or equal than minimum size",
                            HttpServletResponse.SC_BAD_REQUEST);
                }
