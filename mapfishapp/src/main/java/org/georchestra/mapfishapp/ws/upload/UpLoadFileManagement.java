...
public class UpLoadFileManagement {
	...
	/**
	 * a zip file is unzipped to a temporary place and *.shp, *.mid, *.tab files are
	 * looked for at the root of the archive. If several SHP or several MIF or
	 * several TAB files are found, the error message is "multiple files"
	 *
	...
	public void writeFeatureCollectionAsJSON(Writer writer, final CoordinateReferenceSystem crs) throws Exception {
		...
		try {
			...
			final String message = "Failed reading " + FilenameUtils.getName(fileName) + ".  " + e.getMessage();
			LOG.error(message);
