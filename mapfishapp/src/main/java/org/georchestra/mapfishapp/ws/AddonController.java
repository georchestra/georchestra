...
public class AddonController implements ServletContextAware {
    ...
    /**
     * Returns a dynamically-generated array of addons, previously statically stored
     * into the GEOR-custom.js file.
     *
     * The returned format looks like: [{ "id": "magnifier_0", // unique & stable
     * string identifier for this addon instance "name": "Magnifier", "title": {
     * "en": "Aerial imagery magnifier", "es": "Lupa ortofoto", "fr": "Loupe
     * orthophoto", "de": "Ortofoto Lupe" }, "description": { "en": "A tool which
     * allows to zoom in an aerial image on a map portion", "es": "Una herramienta
     * que permite hacer un zoom sobre una parte del mapa ortofoto", "fr": "Un outil
     * qui permet de zoomer dans une orthophoto sur une portion de la carte", "de":
     * "Utensil erlaubt Zoom mittels orthofoto auf Kartenbereich" } }, ...]
     *
    ...
    public void getAddonFile(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (!officialAddonFile.toAbsolutePath().toString().startsWith(officialWebappPath.toAbsolutePath().toString())) {
    ...
    private JSONArray buildAddonSpecs(String path) {
        ...
        for (int i = 0; i < files.length; i++) {
            File curConfig = new File(
                    String.format("%s%s%s%s%s", path, File.separator, files[i], File.separator, "config.json"));

            if (!curConfig.exists()) {
                LOG.error(String.format("Addon %s does not have a config.json configuration file, skipping it.",
                        files[i]));
                continue;
            }
            try {
                ...
                LOG.error(String.format(
                        "Addon %s does not have a readable config.json configuration file, skipping it.", files[i]), e);
                continue;
    ...
    /**
     * Constructs the array of addons specifications. This is similar with what was
     * defined statically in the GEOR_custom.js file. It first scans the official
     * addons, before scanning (if activated and available) the addons from the
     * datadir.
     *
