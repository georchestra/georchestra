...
/**
 * This controller allows to retrieve the list of the WMC contexts hosted in the
 * mapfishapp datadir.
 *
...
public class ContextController implements ServletContextAware {
    ...
    /**
     * Setter for the geOrchestraConfiguration bean, used mainly for testing
     * purposes.
     *
    ...
    private JSONObject getContextInfo(File f) throws Exception {
        ...
        if (!imagePath.isDirectory()) {
            LOG.error("No \"images\" subdirectory found into \"" + pathCtx
                    + "\". Please check your setup, using default image for context \"" + title + "\".");
        } else {
            Collection<File> files = FileUtils.listFiles(imagePath,
                    new String[] { "PNG", "png", "jpeg", "JPEG", "jpg", "JPG" }, false);
     ...
     * provided next to the context file (i.e. context.xml and context.wmc), it is
     * possible to limit its access to the defined roles.
     *
     ...
     * </pre>
     *
     * The purpose is not to prevent the user from accessing the context (since if
     * the user is knowing the context name, it is trivial to call the controller
     * and get the context served by the getContext() method below), but to at least
     * hide it from the mapfishapp interface.
     *
     * @param pathContext the path to the WMC context file
     * @param title       the title
     * @return a JSON array containing the roles, or an empty array if all roles are
     *         allowed.
     * @throws Exception
     */
    ...
    public JSONArray getContexts() throws Exception {
        ...
        if (ctxDir != null) {
            ...
            if (!ctxCtxPath.isDirectory()) {
                LOG.error("No context sub-directory found in \"" + ctxDir
                        + "\". Returning an empty array of contexts. Please check your setup.");
                return ret;
            }
    ...
    public void getContext(HttpServletRequest request, HttpServletResponse response, @PathVariable String contextName)
        ...
        if (ctxDir != null) {
            try {
                byte[] ret = FileUtils.readFileToByteArray(
                        new File(ctxDir, File.separator + "contexts" + File.separator + contextName + ".wmc"));
    ...
    public void getContextImage(HttpServletRequest request, HttpServletResponse response,
        ...
        if (!imgFmt.equalsIgnoreCase("png") && !imgFmt.equalsIgnoreCase("jpg") && !imgFmt.equalsIgnoreCase("jpeg")) {
        ...
        if (ctxDir != null) {
            try {
                File imgFile = new File(ctxDir, File.separator + "contexts" + File.separator + "images" + File.separator
                        + contextName + "." + imgFmt);
