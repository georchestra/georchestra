...
public class SLDClassifier {
    ...
    private void doClassification() throws DocServiceException {
        try {
            ...
            // We need a display (Symbolizers) and a value (Filters) fatories to generate a
            // SLD file
            ...
            if (_command.getClassifType() == E_ClassifType.CHOROPLETHS
                ...
                if (getDataType(wfs) == String.class) {
                    ...
                    throw new DocServiceException("Classification on continous values (" + _command.getClassifType()
                            ...
                            + " Use unique values classification instead.", HttpServletResponse.SC_BAD_REQUEST);
                } else if ((getDataType(wfs) != Double.class) && (getDataType(wfs) != Float.class)
                        && (getDataType(wfs) != Integer.class) && (getDataType(wfs) != Long.class)
    ...
    /**
     * Creates a FeatureTypeStyle (core part of a SLD file). It is composed by Rules
     * (tag <sld:Rule>) and each Rule can contain one Filter (filters values) and
     * one Symbolizer (what's displayed). It Needs 2 factories: <br />
    ...
    private FeatureTypeStyle createFeatureTypeStyle(I_FilterFactory filterFact, I_SymbolizerFactory symbolizerFact) {
        ...
        /*
         * This piece of code can be added to add a stroke around the polygons. This
         * rule does not include a filter, therefore, it is globally applied.
         */

        /*
         * StyleBuilder styleBuilder = new StyleBuilder(); Rule rule =
         * styleFactory.createRule(); Stroke stroke = styleFactory.createStroke(
         * styleBuilder.literalExpression(Color.BLACK), // color
         * styleBuilder.literalExpression(1), // width
         * styleBuilder.literalExpression(1.0)); // opacity LineSymbolizer ls =
         * styleFactory.createLineSymbolizer(stroke, ""); rule.setSymbolizers(new
         * Symbolizer[] {ls}); fts.addRule(rule);
         */
        ...
        for (Map.Entry<String, UsernamePasswordCredentials> cred : _credentials.entrySet()) {
            ...
            try {
                if (host.equalsIgnoreCase(targetHost)
                        || InetAddress.getByName(host).getHostAddress().equals(ipAddress)) {
    ...
    /**
     * Extract values as Double from the given features and property name. Executes
     * the same job as
    ...
    /**
     * Extract values as String from the given features and property name. Executes
     * the same job as
