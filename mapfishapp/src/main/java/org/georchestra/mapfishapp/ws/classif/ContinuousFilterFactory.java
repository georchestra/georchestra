...
public class ContinuousFilterFactory implements I_FilterFactory {
    ...
    /**
     * Provides a way to access Filter objects that are created on the fly from the
     * intervals found by the classification
     */
    public class InternalIterator implements Iterator<Filter> {
        ...
        public Filter next() {
            ...
            org.opengis.filter.Filter _filter = filterFactory.between(filterFactory.property(_propertyName),
                    filterFactory.literal(lowerBoundary), filterFactory.literal(upperBoundary));
