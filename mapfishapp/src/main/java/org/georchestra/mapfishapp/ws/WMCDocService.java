...
/**
 * This service handles the storage and the loading of a wmc file on a temporary
 * directory.
 *
...
public class WMCDocService extends A_DocService {
     ...
     * generated by parent class A_DocService.
     * 
    ...
    protected JSONObject extractsStandardSpecificEntries(InputStream rawDoc) throws Exception {
        ...
        // Extract title
        Node node = (Node) xPath.evaluate("/ViewContext/General/Title", document.getDocumentElement(),
                XPathConstants.NODE);
        if (node != null)
            res.put("title", node.getTextContent());

        // Extract abstract
        node = (Node) xPath.evaluate("/ViewContext/General/Abstract", document.getDocumentElement(),
                XPathConstants.NODE);
        if (node != null)
        ...
        node = (Node) xPath.evaluate("/ViewContext/General/BoundingBox", document.getDocumentElement(),
                XPathConstants.NODE);
        if (node != null) {
    ...
    /**
     * Get the real name of the file given by the user. It is hidden in the id
     * attribute of the ViewContext Node
     * 
     * @param content
