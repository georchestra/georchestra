...
GEOR.Addons.OSMSearchPlaces = Ext.extend(GEOR.Addons.Base, {
	...
	_refreshCbValues: function(event) {
		...
		overpassQuery += '('+this.options.boundingbox.minlat+','+this.options.boundingbox.minlon+','+this.options.boundingbox.maxlat+','+this.options.boundingbox.maxlon+');';
				...
				var onRequestComplete = function(scope) {
					...
					if (requestComplete >= requestCount) {
						...
						for(var i = 0; i < searchResults.length; i++) {
							var displayVal = searchResults[i].getAttribute("display_name").split(scope.options.cutFrom)[0];
							var geometryVal = searchResults[i].getAttribute("geotext");
				...
				for (var i  = 0; i < searchTerms.length; i++) {
					Ext.Ajax.request({
						...
						params: {
							...
							q: searchTerms[i],
							bounded: 1,
							viewbox: this.options.boundingbox.minlon+','+this.options.boundingbox.maxlat+','+this.options.boundingbox.maxlon+','+this.options.boundingbox.minlat
						},
						method: 'GET',
						scope: this,
						success: function(respon,opt){
