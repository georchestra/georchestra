...
GEOR.Addons.coordinatesquery = function(map, feature, services) {
    ...
    this.feature.coordinates = {
        lon: this.projfeature.x,
        lat: this.projfeature.y,
        alt: []
    };
    ...
    for (var i = 0; i < this.services.length; i++) {
        ...
        var infoslayers = [];
        Ext.each(service.layers, function(l, i) {
            infoslayers.push(l.name);
        });
        infoslayers.join(",");
        var params = {
            SERVICE: "WMS",
        ...
        onSuccess: function(response) {
            ...
            if (features.length > 0) {
                for (var i = 0; i < features.length; i++) {
                    ...
                    this.feature.coordinates.alt.push({
                        label: metadata.label,
                        metadata: metadata.metadata,
                        z: Ext.util.Format.number(features[i].attributes.altitude, "0.00")
                    });
                }
