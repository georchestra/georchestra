...
public class DocControllerTest {
	...
	/**
	 * Test reject from server when the DocController is accessed with GET and does
	 * not receive the filename in argument
	 */
	@Test
	...
	public void testCSVService() throws Exception {
		...
		JSONObject JSONContent = new JSONObject().put("columns", new JSONArray().put("col1").put("col2")).put("data",
				new JSONArray().put(new JSONArray().put("1").put("2")).put(new JSONArray().put(3).put(4)));
	...
	public void testClassifier() throws Exception {
		...
		JSONObject jsReq = new JSONObject().put("type", "PROP_SYMBOLS")
				.put("wfs_url",
						"https://sdi.georchestra.org/geoserver/wfs?service=WFS&request=GetCapabilities&version=1.0.0")
				.put("layer_name", "geor:sdi").put("attribute_name", "csw_db").put("class_count", classCount)
				.put("min_size", minSize).put("symbol_type", "point").put("max_size", maxSize);
	...
	public void testIndentDataXee() throws Exception {
		...
		// Length expected of the string (without external entity resolution < 150
		// chars)
		assertTrue("XML generated seems too long, vulnerable to XEE attacks ?", ret.length() < 150);
	}
}
