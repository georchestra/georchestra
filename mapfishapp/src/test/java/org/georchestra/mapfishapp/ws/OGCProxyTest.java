...
public class OGCProxyTest {
    ...
    public void testGetFeatureInfo() throws Exception {
        ...
        assertEquals(_response.getErrorMessage(), "application/vnd.ogc.gml", contentType.split(";")[0]); // content type
                                                                                                         // valid
    }
    ...
    public void testGetCapabilities() throws Exception {
        ...
        String contentType = (String) _response.getHeaders("Content-Type").get(0);
        assertEquals(_response.getErrorMessage(), "application/vnd.ogc.wms_xml", contentType.split(";")[0]); // content
                                                                                                             // type
                                                                                                             // valid
    }
    ...
    public void testHeaders() throws Exception {
        ...
        assertEquals("gzip", _response.getHeader("Content-Encoding").toString()); // the service tested returns gzip
                                                                                  // therefore headers must contain
                                                                                  // the gzip one
                                                                                  // to warn browser
    }
    ...
    public void testMalformedUrl() throws Exception {
        ...
        if (!_proxy.isFilteringOnFinalHost()) {
            ...
            assertEquals(_response.getErrorMessage(), 400, _response.getStatus()); // expect reject from proxy : status
                                                                                   // 400, bad request
        }
    }
    ...
    public void testWrongProtocol() throws Exception {
        ...
        assertEquals(_response.getErrorMessage(), 400, _response.getStatus()); // expect reject from proxy : status 400,
                                                                               // bad request
    }
    ...
    private boolean areContentTypeAllValid() {
        ...
        for (String contentType : _proxy.getValidContentTypes()) {
            if (!_proxy.isContentTypeValid(contentType + ";text-encoding")) { // add text encoding (sometimes appended
                                                                              // to the content type)
                return false;
            }
        }
