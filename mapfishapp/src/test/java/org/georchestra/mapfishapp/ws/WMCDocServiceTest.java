...
public class WMCDocServiceTest {
    ...
    public void testXEEOnExtractRealFileName() {
        assumeTrue(
                "file /etc/passwd does not exist, which is unlikely if you are running the testsuite under linux. Skipping test",
        ...
        String ret = (String) ReflectionUtils.invokeMethod(erfn, wmcds, new ByteArrayInputStream(xeeVuln.getBytes()));

        // If no resolution, filename should be null
        assertTrue("title is not null, XEE attack vulnerable ?", StringUtils.isEmpty(ret));
    }
    ...
    public void testWMCParse() throws Exception {
        ...
        assertTrue("Missing or invalid abstract",
                res.getString("abstract").equals("This is the default context "
    ...
    public void testNoTitle() throws Exception {
        ...
        assertTrue("Valid abstract", res.getString("abstract").equals(
                "This is the default context provided for geOrchestra, loading a layer kindly provided by GéoBretagne, data issued from OpenStreetMap and contributors"));
    }
