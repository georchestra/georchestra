...
Ext.define('Ext.grid.plugin.Editing', {
    ...
    initEditTriggers: function() {
        ...
        if (me.triggerEvent == 'cellfocus') {
            ...
            // not trigger a click.
            // This Editor must call the onXxxxFocus methods directly from the prototype on the View just before we edit
            view.onCellFocus = view.onRowFocus = Ext.emptyFn;
            me.doFocusing = true;
    ...
    startEdit: function(record, columnHeader) {
        ...
        if (me.doFocusing) {
            Ext.view.Table.prototype.onRowFocus.call(me.view, context.rowIdx, true);
        }
        me.context = context;
        me.editing = true;
    },
    ...
    getEditingContext: function(record, columnHeader) {
        ...
        // if the client grid is the top level lockable grid, then the view will be an Ext.grid.LockingView, and we will need
        // to select one of the two child views.
        // If the client grid is not lockable (or is one child grid of the lockable grid, this need not be done)
        if (grid.lockable) {
            gridview = columnHeader.ownerCt.lockableInjected ? (columnHeader.locked ? view.lockedView : view.normalView) : view;
        }
