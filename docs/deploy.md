...
Once your system is ready, collect WAR files in a dedicated directory and rename them:
    ...
    mv catalogapp-${VERSION}.war catalogapp.war
    mv downloadform-${VERSION}.war downloadform.war
...
However, this requires the build process to happen on the production host (or a NFS mount).
    ...
    sudo ln -s ~/.m2/repository/org/georchestra/catalogapp/${VERSION}-SNAPSHOT/catalogapp-${VERSION}-SNAPSHOT.war catalogapp.war
    sudo ln -s ~/.m2/repository/org/georchestra/downloadform/${VERSION}-SNAPSHOT/downloadform-${VERSION}-SNAPSHOT.war downloadform.war
