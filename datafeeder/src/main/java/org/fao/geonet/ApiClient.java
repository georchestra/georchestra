...
import org.fao.geonet.auth.HttpBasicAuth;
...
public class ApiClient {
    ...
    private String basePath = "https://localhost:28080/geonetwork";
    ...
    public ApiClient setRequestHeaderAuth(@NonNull String authName, @NonNull HttpHeaders authHeaders) {
        final HttpHeaders headers = new HttpHeaders(authHeaders);

        RequestInterceptor interceptor = template -> {
            headers.forEach(template::header);
        };

        getApiAuthorizations().remove("basicAuth");

        addAuthorization(authName, interceptor);
        return this;
    }
     ...
     * apiClient.buildClient(XYZApi.class); XYZResponse response =
     * api.someMethod(...);
     * 
     * @param <T>         Type
