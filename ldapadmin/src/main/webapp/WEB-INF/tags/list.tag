...
<%@attribute name="items" required="true" type="java.util.Map<java.lang.String, java.lang.String>"%>
	...
	<div id="div-${path}" class="form-group ${status.error ? 'has-error' : '' }">
		...
		<div class="col-lg-8">
			<c:if test="${not empty appendIcon}">
				...
				<form:select path="${path}" cssClass="${empty cssClass ? 'form-control' : cssClass}" onkeyup="${onkeyup}" onchange="${onchange}">
