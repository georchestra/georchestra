...
public class UsersControllerTest {
    ...
    public void testFindAllException() throws DataServiceException {
        Mockito.doThrow(DataServiceException.class).when(ldapTemplate).search(any(DistinguishedName.class), anyString(),
    ...
    public void testFindByUidDataServiceException() throws Exception {
        Mockito.doThrow(DataServiceException.class).when(ldapTemplate).lookup(any(Name.class),
                any(ContextMapper.class));
    ...
    public void testFindByUid() throws Exception {
        ...
        Mockito.when(ldapTemplate.lookup(any(DistinguishedName.class), eq(UserSchema.ATTR_TO_RETRIEVE),
                any(ContextMapper.class))).thenReturn(pmauduit);
    ...
    public void testUpdateUserDataServiceException() throws Exception {
        Mockito.doThrow(DataServiceException.class).when(ldapTemplate).lookup(
                eq(new DistinguishedName("uid=pmauduit,ou=users")), any(String[].class), any(ContextMapper.class));
    ...
    public void testUpdateBadJSON() throws Exception {
        ...
        Mockito.when(ldapTemplate.lookup(any(Name.class), any(String[].class), any(ContextMapper.class)))
    ...
    public void testUpdate() throws Exception {
        ...
        Account fakedAccount = AccountFactory.createBrief("pmauduit", "monkey123", "Pierre", "pmauduit",
                "pmauduit@georchestra.org", "+33123456789", "developer & sysadmin", "dev&ops");
        Mockito.doReturn(fakedAccount).when(ldapTemplate).lookup(any(Name.class), any(String[].class),
                any(ContextMapper.class));
        // Returns the same account when searching it back
        String mFilter = "(&(objectClass=inetOrgPerson)(objectClass=organizationalPerson)"
                + "(objectClass=person)(mail=tomcat2@localhost))";
        ...
        Mockito.doReturn(listFakedAccount).when(ldapTemplate).search(eq(DistinguishedName.EMPTY_PATH), eq(mFilter),
                any(SearchControls.class), any(ContextMapper.class));
