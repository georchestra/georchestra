...
public class NewAccountFormControllerTest {
    ...
    /**
     * General case : creating a user with all the requirements fulfilled.
     *
     * @throws IOException
     */
    ...
    public void testCreateDuplicatedEmail() throws Exception {
        configureLegitFormBean();
        Mockito.doThrow(new DuplicatedEmailException("User already exists")).when(accountDao)
                .insert((Account) Mockito.any(), Mockito.anyString(), Mockito.anyString());
    ...
    public void testCreateUserWithError() throws Exception {
        configureLegitFormBean();
        Mockito.doThrow(new DataServiceException("Something went wrong when dealing with LDAP")).when(accountDao)
                .insert((Account) Mockito.any(), Mockito.anyString(), Mockito.anyString());
    ...
    public void testCreateDuplicatedUid() throws Exception {
        configureLegitFormBean();
        Mockito.doThrow(new DuplicatedUidException("User ID already exists")).when(accountDao)
                .insert((Account) Mockito.any(), Mockito.anyString(), Mockito.anyString());
        ...
        Mockito.doThrow(new DataServiceException("something has messed up")).when(accountDao)
                .generateUid(Mockito.anyString());
        try {
            ctrl.create(request, formBean, "", result, status, UiModel);
        } catch (Throwable e) {
            assertTrue(e instanceof IOException);
    ...
    public void testAccountFormBean() {
        ...
        assertTrue(t.toString()
                .equals("AccountFormBean [uid=123123-21465456-3434, "
                        ...
                        + "password=monkey123, confirmPassword=test, " + "recaptcha_response_field=wrong]"));
    }
