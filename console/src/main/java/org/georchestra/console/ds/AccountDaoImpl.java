...
public final class AccountDaoImpl implements AccountDao {
     ...
     */
    @Override
    public synchronized void insert(final Account account, final String roleID, final String originLogin)
     ...
     */
    @Override
    public synchronized void delete(final String uid, final String originLogin)
    ...
    public Account findByUID(final String uid) throws NameNotFoundException {
        ...
        Account a = (Account) ldapTemplate.lookup(buildDn(uid.toLowerCase()), UserSchema.ATTR_TO_RETRIEVE,
                attributMapper);
        if (a == null)
            throw new NameNotFoundException("Cannot find user with uid : " + uid + " in LDAP server");
        else
    ...
    private void mapToContext(Integer uniqueNumber, Account account, DirContextOperations context) {
        ...
        context.setAttributeValues("objectclass",
                new String[] { "top", "person", "organizationalPerson", "inetOrgPerson", "shadowAccount" });
        ...
        if (account.getManager() != null)
            setAccountField(context, UserSchema.MANAGER_KEY,
                    "uid=" + account.getManager() + "," + this.userSearchBaseDN.toString() + "," + this.getBasePath());
        else
