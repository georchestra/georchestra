...
public class OrgsDao {
	...
	public List<Org> findAll() {
		...
		List<Org> active = ldapTemplate.search(orgSearchBaseDN, filter.encode(),
				new OrgsDao.OrgAttributesMapper(false));
		List<Org> pending = ldapTemplate.search(pendingOrgSearchBaseDN, filter.encode(),
				new OrgsDao.OrgAttributesMapper(true));
	...
	public List<OrgExt> findAllExt() {
		...
		List<OrgExt> active = ldapTemplate.search(orgSearchBaseDN, filter.encode(),
				new OrgsDao.OrgExtAttributesMapper(false));
		List<OrgExt> pending = ldapTemplate.search(pendingOrgSearchBaseDN, filter.encode(),
				new OrgsDao.OrgExtAttributesMapper(true));
		return Stream.concat(active.stream(), pending.stream()).collect(Collectors.toList());
	}
	...
	/**
	 * @return Org instance corresponding to organization of specified user or null
	 *         if no organization is linked to this user
	 * @throws DataServiceException if more than one organization is linked to
	 *                              specified user
	 */
	...
	private void mapToContext(Org org, DirContextOperations context) {
		...
		String seeAlsoValue = LdapNameBuilder.newInstance(orgSearchBaseDN + "," + basePath).add("o", org.getId())
				.build().toString();
