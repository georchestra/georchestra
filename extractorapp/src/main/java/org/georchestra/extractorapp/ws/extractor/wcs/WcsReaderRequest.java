...
/**
 * A convenient class that represents a GetCoverageRequest. The result from
...
public class WcsReaderRequest {
     ...
     * @param username
     */
    protected WcsReaderRequest(String version, String coverage, ReferencedEnvelope bbox,
    ...
    protected WcsReaderRequest(WcsReaderRequest request) {
        this(request.version, request.coverage, request.requestBbox, request.responseCRS, request.groundResolutionX,
                request.format, request.usePost, request.remoteReproject, request.useCommandLineGDAL, request.username,
                request.password);
    }
    ...
    public WcsReaderRequest withFormat(String newFormat) {
        return new WcsReaderRequest(version, coverage, requestBbox, responseCRS, groundResolutionX, newFormat, usePost,
                remoteReproject, useCommandLineGDAL, username, password);
    }

    /**
     * Create a new request based on the current request but with a new CRS. The
     * bbox is reprojected as required
     */
    ...
    /**
     * Convert this request to Geotools parameters. These are required to make a
     * request with the WcsCoverageReader, unless
     * {@link #execute(WcsCoverageReader)} is used instead
     */
    public GeneralParameterValue[] getParameters() {
        ...
        return new GeneralParameterValue[] { usePost, remoteReproject, useCommandLineGDAL, version, coverage, format,
                crs, extent, resultImageParams, username, password };
    }
    ...
    /**
     * Used by this class and tests for getting the parameter data from a geotools
     * parameter
     */
