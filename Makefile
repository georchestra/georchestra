...
docker-build-gn3: docker-pull-jetty
	...
	cd web; \
	mvn -P docker -DskipTests package docker:build -DdockerImageTags=latest
...
docker-build-geoserver: docker-pull-jetty
	...
	cd webapp; \
	mvn clean install docker:build -DdockerImageTags=latest -Pdocker,${GEOSERVER_EXTENSION_PROFILES} -DskipTests
...
docker-build-geoserver-geofence: docker-pull-jetty
	...
	cd webapp; \
	mvn clean install docker:build -DdockerImageTags=latest -Pdocker,geofence,${GEOSERVER_EXTENSION_PROFILES} -DskipTests

docker-build-proxy: build-deps docker-pull-jetty
	mvn clean package docker:build -DdockerImageTags=latest -Pdocker -DskipTests --pl security-proxy

docker-build-cas: build-deps docker-pull-jetty
	mvn clean package docker:build -DdockerImageTags=latest -Pdocker -DskipTests --pl cas-server-webapp

docker-build-console: build-deps docker-pull-jetty
	mvn clean package docker:build -DdockerImageTags=latest -Pdocker -DskipTests --pl console

docker-build-mapfishapp: build-deps docker-pull-jetty
	mvn clean package docker:build -DdockerImageTags=latest -Pdocker -DskipTests --pl mapfishapp
...
docker-build-georchestra: build-deps docker-pull-jetty docker-build-database docker-build-ldap docker-build-geoserver docker-build-gn3
	mvn clean package docker:build -DdockerImageTags=latest -Pdocker -DskipTests --pl extractorapp,cas-server-webapp,security-proxy,mapfishapp,header,console,analytics,geowebcache-webapp,atlas
