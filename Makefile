...
docker-build-georchestra: build-deps docker-pull-jetty docker-build-database docker-build-ldap docker-build-geoserver docker-build-geowebcache docker-build-gn
	mvn clean package docker:build -DdockerImageTags=${BTAG} -Pdocker -DskipTests --pl extractorapp,security-proxy,header,console,analytics,datafeeder
...
deb-build-georchestra: war-build-georchestra build-deps deb-build-geoserver deb-build-geowebcache
	mvn package deb:package -pl datafeeder,datafeeder-ui,security-proxy,header,extractorapp,analytics,console,geonetwork/web -PdebianPackage -DskipTests ${DEPLOY_OPTS}
